  ...
  return (
    <div key={user._id} className='p-4 pt-6 flex flex-col justify-between w-72 shadow border border-gray-200 rounded-md'>
        <div className='text-center'>
            ...
            {user.username && <p className='text-sm text-slate-500 font-light'>@{user.username}</p>}
            {user.bio && <p className='text-sm text-gray-600 mt-2 text-center px-4'>{user.bio}</p>}
        </div>
        ...
        <div className='flex mt-4 gap-2'>
            {/*FOLLOW BUTTON*/}
            <button onClick={handleFollow} disabled={currentUser?.following.includes(user._id)} className='w-full py-2 rounded-md flex justify-center items-center gap-2 bg-linear-to-r from-indigo-500 
            to-purple-600 hover:from-indigo600 hover:to-purple-700 active:scale-95 transition text-white cursor-pointer'>
